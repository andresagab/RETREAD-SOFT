    ...
    $scope.loadData = function () {
        ...
        OrdenesServiciosFactory.getData('ordenesServicioJSON.php').then(function (r) {
            if (validStatusHttpResponse(r, true)) {
                ...
                $scope.setCurrentPage(null);
                showToastPrincipalTime(true, r.data.data.length + ' registros cargados', 2500);
            }
        });
    }
            ...
            if (isNummber(valueSearch)) {
                ...
                OrdenesServiciosFactory.getDirectSearch('ordenesServicioJSON.php', $scope.module.dataDirectSearch).then(function (r) {
                    ...
                    if (validStatusHttpResponse(r, true)) {
                        ...
                        $scope.initValues(r.data.data);
                        $scope.setCurrentPage(null);
                        showToastPrincipalTime(true, 'Se encontraron ' + r.data.data.length + ' resultados.', 2500);
                    }
                });
            } else {
                showToastPrincipalTime(true, 'No se puede buscar una palabra o letras como números de orden o rp', 3000);
                $scope.module.dataDirectSearch.directSearch = false;
                $scope.module.dataDirectSearch.valueSearch = null;
            }
        } else {
            showToastPrincipalTime(true, 'Para buscar ingresa el número de orden de servicio o el número de rp', 3000);
            $scope.module.dataDirectSearch.directSearch = false;
            $scope.module.dataDirectSearch.valueSearch = null;
        }
    };
