panamApp.controller('usoInsumosProceso', function ($scope, configuracionGlobal, $http, $location, $timeout){
    ...
    $scope.page = {
        components: {
            ...
            btnAbrirPuestoTrabajo: false,
            btnSendForm: true,
            spnPuestosTrabajo: false,
    ...
    $scope.setNumeroProceso=function (_numeroProceso){
        if (validarInput(_numeroProceso)) {
            $scope.page.data.numeroProceso = _numeroProceso;
            // if (_numeroProceso!=3 && _numeroProceso!=6) $scope.page.components.btnUsarVariosInsumos = true;
        }
            ...
            if (validarInput(_id)){
                $http({
                    ...
                    if (r.length>0 && r[0]!=null){
                        $scope.page.components.btnAbrirPuestoTrabajo = true;
                        ...
                        $scope.page.components.btnAbrirPuestoTrabajo = false;
                    }
                }).error(function (r){
                    showToast(true, 'Error 404: No se encontro el archivo y/o no se pudo conectar al servidor');
                });
    ...
    $scope.cargarUsosInsumosProceso =function (){
        if (validarInput($scope.page.data.proceso.id) && validarInput($scope.page.data.numeroProceso)){
            ...
            } else showToast(true, 'Cero insumos o herramientas usadas, recuerda hacer el registro de los mismos');
        }
    };
    //REVIZAR
    $scope.validarUsosInsumosProceso = function (){
        ...
        if ($scope.page.data.usosInsumoProceso.length>0) {
            valid = true;
            $scope.page.components.alertPrincipal.status = false;
        } else {
            $scope.page.components.alertPrincipal.status = true;
            $scope.page.components.alertPrincipal.color = 'warning';
            $scope.page.components.alertPrincipal.mjs = 'Debes registrar los usos de los insumos o las herramientas';
            $("#puestoTrabajo").focus();
        }
        return valid;
    };
    ...
    $scope.seleccionarPuestoTrabajo = function (){
        if ($scope.validarUsosInsumosProceso()) {
            $scope.page.data.puestoTrabajo.id = $scope.page.data.usosInsumoProceso[0].idpuestotrabajoinsumo;
            $scope.page.components.spnPuestosTrabajo = true;
            $scope.page.components.btnSendForm = false;
            $("#puestoTrabajo").val($scope.page.data.puestoTrabajo.id);
            $scope.cargarPuestoTrabajo(true, null, $scope.page.data.puestoTrabajo.id);
        } else {
            $scope.page.components.spnPuestosTrabajo = false;
            $scope.page.components.btnSendForm = true;
            $("#puestoTrabajo").val($scope.page.data.puestoTrabajo.id);
            $scope.cargarPuestoTrabajo(true, null, $scope.page.data.puestoTrabajo.id);
        }
    };
    ...
    $scope.cargarInsumosPuestoTrabajo = function (_IdPuestoTrabajo){
        ...
        if (validarInput(_IdPuestoTrabajo)){
            ...
            $http({
                url: configuracionGlobal.scripts + "/insumosPuestoTrabajoRanking.php",
                method: 'GET',
                params: {
                    ...
                    asc: false,
                    extras: true,
                    numberProcess: $scope.page.data.numeroProceso
                }
            }).success(function (r){
    ...
    $scope.seleccionarAllInsumos = function (){
        if ($scope.page.components.chksInsumos) {
                ...
                if ($scope.page.data.usosInsumoProceso.length>0){
                    for (var i = 0; i < $scope.page.data.usosInsumoProceso.length; i++) {
                        for (var j = 0; j < $scope.page.data.insumosId.length; j++) {
                            if ($scope.page.data.insumosId.indexOf($scope.page.data.usosInsumoProceso[i].idinsumopuestotrabajo)!=-1) $scope.page.data.insumosId.splice($scope.page.data.insumosId.indexOf($scope.page.data.usosInsumoProceso[i].idinsumopuestotrabajo), 1);
                        }
                        for (var x = 0; x < $scope.page.data.insumos.length; x++) {
                            if ($scope.page.data.insumos[x].id===$scope.page.data.usosInsumoProceso[i].idinsumopuestotrabajo) {
            ...
            if ($scope.page.data.insumos!=null){
                if ($scope.page.data.insumos.length>0){
                    $scope.page.data.insumosId = [];
                    for (var i = 0; i < $scope.page.data.insumos.length; i++) $scope.page.data.insumos[i].chk = false;
    ...
    $scope.separarIdInsumo = function (_chk, _IdInsumo){
        if ($scope.page.data.insumosId.indexOf(_IdInsumo)!=-1) $scope.page.data.insumosId.splice($scope.page.data.insumosId.indexOf(_IdInsumo), 1);
    ...
    $scope.validarInsumos = function (){
        ...
        if ($scope.page.data.insumosId.length>0){
            for (var i = 0; i < $scope.page.data.insumosId.length; i++) {
                if ($scope.page.data.insumosId[i]!=null) {
                    valid = true;
        ...
        if (validarInput(_idInsumo)){
            ...
            if ($scope.validarDatosProceso() && $scope.validarDatosEmpleado() && _metodo!=null){
                ...
                $http({
                    ...
                    $scope.usoInsumo = {};
                    //$scope.page.data.insumos = [];
                    $scope.cargarProceso(true, null, $scope.page.data.proceso.id, 'getSimpleRaspadoJSON');
                    $scope.cargarUsosInsumosProceso();
                    $scope.page.components.chksInsumos = false;
                    $scope.page.data.insumosId = [];
                    $scope.seleccionarAllInsumos();
