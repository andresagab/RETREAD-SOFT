panamApp.controller('preparacion', function ($scope, $interval) {
    ...
    $scope.initProcces = function () {
        window.location = 'principal.php?CON=system/pages/preparacionActualizar.php&idLlanta=' + document.getElementById('txtIdLlanta').value + '&idPastProcces=' + document.getElementById('txtPastProcces').value + '&accion=initProcces';//review
    };

    $scope.toProcess = function () {
        window.location = 'principal.php?CON=system/pages/raspadoFormulario.php&id=' + document.getElementById('txtIdLlanta').value + '&accion=initProcess';//review
    };

    $scope.resetProcces = function (id){
        window.location = 'principal.php?CON=system/pages/preparacionActualizar.php&id=' + id + '&idLlanta=' + document.getElementById('txtIdLlanta').value + '&idPastProcces=' + document.getElementById('txtPastProcces').value + '&accion=resetProcces';//review
    };
    ...
    $scope.setTimeToGo = function () {
        ...
        $interval(function () {
            ...
            // if (Math.round(minutes)>=1.5) location.reload();
            if (Math.round(minutes) >= 1 && Math.round(seconds) >= 30) location.reload();
        }, 1000);
    };
    ...
    $scope.backPage = function () {
        window.location = 'principal.php?CON=system/pages/llantasPreparacion.php';
    };
