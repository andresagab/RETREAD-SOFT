panamApp.controller('usosInsumosCortesBandas', function ($scope, configuracionGlobal, $http, $location, $timeout){
    ...
    $scope.seleccionarPuestoTrabajo=function (){
        if ($scope.validarUsosInsumosProceso()) {
            ...
            $("#puestoTrabajo").val($scope.puestoTrabajo.id);
            $scope.cargarPuestoTrabajo($scope.puestoTrabajo.id);
    ...
    $scope.cargarInsumosPuestoTrabajo=function (_IdPuestoTrabajo){
        ...
        if (_IdPuestoTrabajo!=null && _IdPuestoTrabajo!='' && _IdPuestoTrabajo!='#'){
            ...
            if ($scope.html.supply_searcher !== null && $scope.html.supply_searcher.length >= 3)
            {
                ...
                $http({
                    ...
                    method: 'GET',
                    params: {
                        metodo: 'search_supplies_in_work_area',
                        idPT: _IdPuestoTrabajo,
                        supply_searcher: $scope.html.supply_searcher
                    }
                }).success(function (r){
                    $scope.html.spinnerCargaDialogo=false;
                    if (r.length>0){
            ...
            // else, show toast with alert
            else
                showToast(true, 'Para realizar la búsqueda asegurate de digitar por lo menos 3 carácteres.');
    ...
    $scope.registrarNovedad=function (form){
        ...
        if (form.$valid && $scope.validarDatosEmpleado() && $scope.validarDatosPuestoTrabajo() && validarInput($scope.novedadPT.novedad)){
            $scope.html.spinnerCargaDialogo=true;
