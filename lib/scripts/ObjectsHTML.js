...
function validStatusHttpResponse(httpResponseData, executeToast) {
    let status = false;
    let mjsToast = "";
    if (httpResponseData != null && httpResponseData != undefined) {
        if (httpResponseData.status != null && httpResponseData.status != undefined) {
            status = true;
            switch (httpResponseData.status) {
                case 200:
                    mjsToast = "La solicitud fue procesada con éxito.";
                    break;
                case 201:
                    mjsToast = "La solicitud ha tenido éxito y se ha creado un nuevo recurso como resultado de ello.";
                    break;
            }
        }
    }
