...
/**
 * Descripcion de la clase Marca_Llanta:
 *
...
class Marca_Llanta {
    ...
    function __construct($campo, $valor, $filtro, $orden){
    	...
    	if ($campo!=null) {
            if (is_array($campo)){
                ...
                $sql="select id, nombre, descripcion, fechaRegistro from {$P}marca_llanta where $campo=$valor $filtro $orden";
    ...
    public function grabar() {
        $P='';
        $sql="insert into {$P}marca_llanta (nombre, descripcion, fechaRegistro) values ('$this->nombre', '$this->descripcion', '$this->fechaRegistro')";
        Conector::ejecutarQuery($sql, null);
    }

    public function modificar() {
        $P='';
        $sql="update {$P}marca_llanta set nombre='$this->nombre', descripcion='$this->descripcion' where id=$this->id";
        Conector::ejecutarQuery($sql, null);
    }

    public function eliminar() {
        $P='';
        $sql="delete from {$P}marca_llanta where id=$this->id";
        Conector::ejecutarQuery($sql, null);
    }
    ...
    public static function getLista($filtro, $orden) {
        ...
        $sql="select id, nombre, descripcion, fechaRegistro from {$P}marca_llanta $filtro $orden";
        return Conector::ejecutarQuery($sql, null);
    }
    
    public static function getListaEnObjetos($filtro, $orden) {
        $datos= Marca_Llanta::getLista($filtro, $orden);
        $objetos=array();
        for ($i = 0; $i < count($datos); $i++) {
            $objetos[$i]=new Marca_Llanta($datos[$i], null, null, null);
        }
        return $objetos;
    }
    
    public static function getDatosEnOptions($filtro, $orden, $predeterminado) {
        $datos= Marca_Llanta::getListaEnObjetos($filtro, $orden);
    ...
    public static function getObjetoJSON($campo, $valor, $filtro, $orden) {
        $objeto=new Marca_Llanta($campo, $valor, $filtro, $orden);
        $JSON=array();
        $arreglo=array();
        ...
        $arreglo['statusDelete']=getStatusDelete($objeto->getId(), ['llanta'], 'idmarca');
