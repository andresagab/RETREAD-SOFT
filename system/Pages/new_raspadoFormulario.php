...
if (isset($_GET['idInspeccion'])) header ("Location: principal.php?CON=system/Pages/raspadoActualizar.php&idInspeccion={$_GET['idInspeccion']}&accion=preRegistro");
else {
    if (isset($_GET['id'])) {
        $objeto=new Raspado('id', $_GET['id'], null, null);
        $inspeccionInicial=$objeto->getInspeccion();
        ...
        $objeto->setIdInspeccion($_GET['idInspeccion']);
        $inspeccionInicial=$objeto->getInspeccion();
        $llanta=$inspeccionInicial->getLlanta();
        $servicio=$llanta->getServicio();
        $accion="Registrar";
    ...
    if ($objeto->getCinturon()) $checked='checked';
    else $checked='';

    # if object of process have saved data and the status of process is 'prs', then redirect to end process form
    if ($objeto->getId() && validVal($objeto->getAnchoBanda()) && validVal($objeto->getFoto()) && $objeto->getEstado() === 'prs')
        header ("Location: principal.php?CON=system/Pages/raspadoFormularioFinalizar.php&id=" . $objeto->getId());
...
# if process object have id
if (validVal($object->getId()))
{
    # if process haven't eployee id and status is 'prs', then enable form
    if (!validVal($object->getIdEmpleado()) && $object->getEstado()==='prs')
    {
        ...
        <div class="col-md-12" ng-controller="usosInsumosPuestoTrabajo">
            ...
            <div class="col-md-4" >
                <strong class="text mdl-color-text--blue control-label text-uppercase"><h2><?=$accion?> raspado</h2></strong>
            </div>
            <div class="col-md-4" >
                    ...
                    <form class="form-horizontal" name="formulario" method="POST" action="principal.php?CON=system/Pages/raspadoActualizar.php" enctype="multipart/form-data">
                        ...
                        <div class="col-md-8">
                            <div class="form-group">
                                <div class="input-group">
                                    <span class="input-group-addon">* Puesto de trabajo:</span>
                                        <select ng-disabled="html.spnListaPuestosTrabajo" class="form-control has-primary input-group-sm" name="idPuestoTrabajo" id="puestoTrabajo" ng-model="html.idPuestoTrabajo" ng-change="cargarPuestoTrabajo(html.idPuestoTrabajo)"><?= Puesto_Trabajo::getDatosEnOptions("proceso=1", null, null)?></select>
                                ...
                                <div class="input-group">
                                    ...
                                    <span class="input-group-addon">
                                        ...
                                        <label class="mdl-switch mdl-js-switch mdl-js-ripple-effect" for="cinturon">
                                            <input type="checkbox" id="cinturon" class="mdl-switch__input" name="cinturon" <?=$checked?>>
                                        </label>
                                    </span>
                                    <span class="input-group-addon"><label id="textoEstado"></label></span>
                                </div>
                            </div>
                            <div class="form-group" id="divSpinner">
                                <div class="input-group" ng-init="cinturonCantidad=1">
                                    <span class="input-group-addon">Cantidad:</span>
                                    <label class="input-group-addon" for="cantidadCinturon">
                                        <input class="mdl-slider mdl-js-slider" id="cantidadCinturon" name="cinturonCantidad" type="range" min="1" max="1" value="1" tabindex="0" ng-model="cinturonCantidad">
                                    </label>
                                    <span class="input-group-addon"><label id="textoCantidadCinturon">{{ cinturonCantidad }}</label></span>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="input-group">
                            ...
                            <div class="hidden">
                                ...
                                <div class="col-md-12"><input type="hidden" name="idInspeccion" value="<?=$objeto->getIdInspeccion()?>"></div>
                                                        ...
                                                        <table class="mdl-data-table mdl-js-data-table">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th ng-click="orden='foto'">
                                                                        <span class="fa fa-camera-retro"></span>
                                                                    </th>
                                                                    <th ng-click="orden='insumo[0].nombrePuc'">Insumo</th>
    ...
    # else, if process have employee, photo and status is 'prf'
                ...
                <div class="panel-body">
                    <div class="col-sm-6 col-md-6 col-lg-6">
                        ...
                        <div class="col-sm-12 col-md-12 col-lg-12" id="paddinTop20" align="left">
                            ...
                            <p>
                                <span class="text-uppercase">NUMERO REENCAUCHE: </span><span class="text-muted"><?= $object->getInspeccion()->getNumeroRencauche() ?></span>
                            </p>
                            <p>
                            ...
                            <p>
                                <span class="text-uppercase">¿RETIRO DE CINTURON?: </span><span class="text-muted"><?= $object->getNombreCinturon() ?></span>
                            </p>
                            <p>
                                <span class="text-uppercase">CANTIDAD CINTURON: </span><span class="text-muted"><?= $object->getCinturonCantidad() ?></span>
                            </p>
                            <p>
                            ...
                            <p>
                                <span class="text-uppercase">TIEMPO DE EJECUCIÓN: </span><span class="text-muted"><?= getDiffTiempoString($object->getInspeccion()->getFechaRegistro(), $object->getFechaRegistro()) ?></span>
                            </p>
                        </div>
                        <div class="col-sm-12 col-md-12 col-lg-12" id="paddinTop20" align="center">
                            <a class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-color--red-400 mdl-color-text--white" type="button" href="principal.php?CON=system/pages/retread_tires.php&wsn=2">REGRESAR AL LISTADO DE LLANTAS</a>
                            <a class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-color--orange-500 mdl-color-text--white" href="principal.php?CON=system/pages/ordenesServicioFormulario.php&id=<?= $object->getInspeccion()->getLlanta()->getIdServicio() ?>">Regresar a la orden de servicio</a>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-6 col-lg-6">
                        <div class="col-sm-12 col-md-12 col-lg-12" align="left">
...
# else, redirect to unknown data
else
    header('Location: principal.php?CON=system/pages/unknowData.php');
...
<script>
    $(document).ready(function(){
        $("#verDetalles").click();
        validarDivCantidadCinturon();
        $("#divSpinner").hide("slow");
    });
    
    $("#cinturon").click(function(){
        validarDivCantidadCinturon();
    });
    
    function validarDivCantidadCinturon(){
        valido=false;
        if(document.getElementById("cinturon").checked) {
            document.getElementById("textoEstado").innerHTML='Si';
            $("#divSpinner").show("slow");
        } else {
            document.getElementById("textoEstado").innerHTML='No';
            $("#divSpinner").hide("slow");
        }
        return valido;
    }
